...
import static cn.example.agent.demo.enums.AgentNameEnum.ORDER_SUCCESS_RATE;
...
public class OrderSuccessRateConfig {
    ...
    @Bean
    public ReactAgent orderSuccessRateAgent(@Qualifier("qwenChatModel") ChatModel qwenChatModel) {
        return ReactAgent.builder()
                .name(ORDER_SUCCESS_RATE.getText())
                .description("分析客户历史订单成功率")
                .model(qwenChatModel)
                .systemPrompt(ORDER_SUCCESS_RATE_PROMPT)
                .instruction("""
                    客户ID: {customerId} 
                    分析客户订单成功率。
                    """)
