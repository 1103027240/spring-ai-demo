...
import static cn.example.agent.demo.enums.AgentNameEnum.REFUND_RATE_CHECK;
...
public class RefundRateCheckConfig {
    ...
    @Bean
    public ReactAgent refundRateCheckAgent(@Qualifier("qwenChatModel") ChatModel qwenChatModel) {
        return ReactAgent.builder()
                .name(REFUND_RATE_CHECK.getText())
                .description("分析客户历史订单退款率")
                .model(qwenChatModel)
                .systemPrompt(REFUND_RATE_CHECK_PROMPT)
                .instruction("""
                    客户ID：{customerId} 
                    分析客户历史订单退款率。
                    """)
