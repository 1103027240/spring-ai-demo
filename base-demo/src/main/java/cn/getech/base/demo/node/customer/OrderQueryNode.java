...
public class OrderQueryNode implements NodeActionWithConfig {
    ...
    private Map<String, Object> parseAIResponse(String aiResponse) {
        ...
        try {
            ...
            log.warn("【订单查询节点】JSON解析失败，使用简化解析: {}", e.getMessage());
    ...
    private Map<String, Object> parseAIResponseSimple(String aiResponse) {
        ...
        try {
            // 提取 queryType
            Pattern pattern = Pattern.compile("\"queryType\"\\s*:\\s*\"([^\"]+)\"");
            Matcher matcher = pattern.matcher(aiResponse);
            if (matcher.find()) {
                result.put(QUERY_TYPE, matcher.group(1));
            }

            // 提取 orderNumber
            pattern = Pattern.compile("\"orderNumber\"\\s*:\\s*\"([^\"]*)\"");
            matcher = pattern.matcher(aiResponse);
            if (matcher.find()) {
                String orderNumber = matcher.group(1);
                if (!"null".equalsIgnoreCase(orderNumber) && StrUtil.isNotBlank(orderNumber)) {
                    result.put(ORDER_NUMBER, orderNumber);
                }
            }

            // 提取 orderStatus
            pattern = Pattern.compile("\"orderStatus\"\\s*:\\s*\"([^\"]*)\"");
            matcher = pattern.matcher(aiResponse);
            if (matcher.find()) {
                String orderStatus = matcher.group(1);
                if (!"null".equalsIgnoreCase(orderStatus) && StrUtil.isNotBlank(orderStatus)) {
                    result.put(ORDER_STATUS, orderStatus);
                }
            }

            // 提取 productInfo
            pattern = Pattern.compile("\"productInfo\"\\s*:\\s*\"([^\"]*)\"");
            matcher = pattern.matcher(aiResponse);
            if (matcher.find()) {
                String productInfo = matcher.group(1);
                if (!"null".equalsIgnoreCase(productInfo) && StrUtil.isNotBlank(productInfo)) {
    ...
    private Map<String, Object> buildQueryParams(Map<String, Object> extractedInfo) {
        ...
        // 订单号
        if (extractedInfo.containsKey(ORDER_NUMBER) && extractedInfo.get(ORDER_NUMBER) != null) {
            queryParams.put(ORDER_NUMBER, extractedInfo.get(ORDER_NUMBER));
        }

        // 订单状态
        if (extractedInfo.containsKey(ORDER_STATUS) && extractedInfo.get(ORDER_STATUS) != null) {
            queryParams.put(STATUS, extractedInfo.get(ORDER_STATUS));
        }

        // 时间范围
        if (extractedInfo.containsKey(TIME_RANGE) && extractedInfo.get(TIME_RANGE) != null) {
            Map<String, String> timeRange = (Map<String, String>) extractedInfo.get(TIME_RANGE);
        ...
        queryParams.put(ORDER_DESC, "DESC");
    ...
    private Map<String, Object> processQueryResult(List<Map<String, Object>> orders, Map<String, Object> extractionInfo) {
        Map<String, Object> result = new HashMap<>();
        String queryType = (String) extractionInfo.get(QUERY_TYPE);
        ...
        // 生成摘要
        String summary = generateOrderSummary(orders, extractionInfo);
     ...
     * 生成订单摘要
     */
    private String generateOrderSummary(List<Map<String, Object>> orders, Map<String, Object> extractionInfo) {
        StringBuilder summary = new StringBuilder();

        if (CollUtil.isEmpty(orders)) {
            summary.append("未找到符合条件的订单。");
        } else {
            summary.append("找到【").append(orders.size()).append("】个订单：\n");
            for (int i = 0; i < Math.min(orders.size(), 3); i++) {
                Map<String, Object> order = orders.get(i);
                summary.append(i + 1).append(". ")
                        .append(order.get(ORDER_NUMBER)).append(" - ")
                        .append(OrderStatusEnum.getDescription((Integer) order.get(STATUS))).append("\n");
            }
            if (orders.size() > 3) {
                summary.append("... 还有【").append(orders.size() - 3).append("】个订单");
            }
        }
