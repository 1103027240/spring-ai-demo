...
public class InventorySalesAgentConfig {
    ...
    private static final String SQL_PROMPT = """
        ...
        ## 规则
        1. 匹配技能 → 查看SKILL.md意图映射表 → 获取SQL → 调用executeSql工具执行 → 原样返回执行结果
        2. 不匹配任何技能 → 直接返回："抱歉，当前系统不支持该业务查询。"
        3. 只能使用上述表名，禁止使用其他表名
        4. 必须调用executeSql工具执行SQL
        5. 所有SQL必须加 LIMIT 10
        6. 禁止对结果进行修饰、格式化、总结、解释，必须原样返回executeSql的输出
        """;
    ...
    private static final String INVENTORY_PROMPT = """
        ...
        ## 规则
        1. 匹配业务 → 查看SKILL.md获取SQL → 调用executeSql工具执行 → 原样返回执行结果
        2. 不匹配 → 直接返回："抱歉，当前库存系统不支持该业务查询。支持：库存状态、补货提醒、入库出库等。"
        3. 只能使用上述表名，禁止使用其他表名
        4. 必须调用executeSql工具执行SQL
        5. 所有SQL必须加 LIMIT 10
        6. 禁止对结果进行修饰、格式化、总结、解释，必须原样返回executeSql的输出
    ...
    private static final String SALES_PROMPT = """
        ...
        ## 规则
        1. 匹配业务 → 查看SKILL.md获取SQL → 调用executeSql工具执行 → 原样返回执行结果
        2. 不匹配 → 直接返回："抱歉，当前销售系统不支持该业务查询。支持：销售业绩、产品排行、客户分析、订单查询等。"
